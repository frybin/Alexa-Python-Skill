...
from rss_skill import app, db
from rss_skill.forms import AddFeedForm, EditFeedForm
...
@app.route('/delete_feed/<string:rss_id>', methods=['POST'])
def delete_feed(rss_id):
    feed = Feed.query.filter_by(rss_i=rss_id).first()
...
@app.route('/edit_feed/<string:rss_id>', methods=['GET', 'POST'])
def edit_feed(rss_id):
    form = EditFeedForm()
    feed = Feed.query.filter_by(rss_i=rss_id).first()
    if form.validate_on_submit():
